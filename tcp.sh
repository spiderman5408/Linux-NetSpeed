...
sh_ver="1.3.1"
github="raw.githubusercontent.com/spiderman5408/Linux-NetSpeed/master"
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/spiderman5408/Linux-NetSpeed/master/bbr/tcp_nanqinlang.c
		echo "obj-m := tcp_nanqinlang.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc-4.9
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://github.com/spiderman5408/lotServer/raw/master/Install.sh) install
	start_menu
}
...
remove_all(){
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://github.com/spiderman5408/lotServer/raw/master/Install.sh) uninstall
	fi
	clear
	echo -e "${Info}:清除加速完成。"
	sleep 1s
